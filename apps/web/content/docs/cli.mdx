...
`kortix marketplace list --type agent` lists agents. Installing an agent adds
its `.kortix/opencode/agents/<name>.md` file and an `agents.<name>` entry in
kortix.yaml with the connectors, secrets, skills, and permissions it declares —
`kortix marketplace show <id> --json` prints that grant as `.agent.governance`.
To grant less, add the agent from the web marketplace and uncheck what you do
not want.
