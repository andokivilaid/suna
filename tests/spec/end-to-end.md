...
`MKTP-1` `GET /marketplace/items {query?,type?}` → auth → 200 `{items:[{id,registry,name,type,title,description,categories,capabilities,dependencies,fileCount,managedBy?,updatePolicy?}]}` (catalog includes the minimal Kortix runtime skills, optional General Knowledge Worker skills such as `pdf`, and curated bundles; the default starter does not ship the GKW pack; `?query=`/`?type=` filter). Browse types are skills, agents, and projects: `?type=agent` returns only `registry:agent` items, and an agent's `capabilities` include the connectors and secrets of its kortix.yaml grant.
`MKTP-2` `GET /marketplace/items/:id` → auth → 200 item detail (`files`, `readme`, `capabilities`, managed metadata when applicable); an agent also returns `agent {name,file,frontmatter,prompt,governance,governanceSource,templateId?,governanceYaml}`; unknown id → 404.
...
`MKTP-11` A project writer starts an agent-driven marketplace installation session. For an agent, the optional `grants` body is the approved capability set and may only narrow the agent's declared kortix.yaml grant. Missing items, unknown items, undeclared or malformed grants, grants on a non-agent item, and unauthorized callers are rejected.
